...
module.exports = (Sequelize, patches) => {
  ...
  const { attributes, isModelInstance, uppercaseFirst } = patches;
  ...
  function convertHierarchy(results, model, parent) {
    const { hierarchy } = model;
    const { primaryKey, foreignKey } = hierarchy;
    const childrenAccessor = hierarchy.childrenAs;
    const descendentsAccessor = hierarchy.descendentsAs;
    const throughAccessor = hierarchy.through.name;
