...
module.exports = (Sequelize, patches) => {
  ...
  const { co, coAll } = makeCo(Sequelize);
  const hooksModel = hooksModelFn(Sequelize, patches);
  const { singularize, pluralize } = Sequelize.Utils;
  ...
  function isHierarchy(options) {
    ...
    options = {
      ...
      ...sequelize.options.hierarchy || {},
      ...options,
    };

    const {
      primaryKey, as, ancestorsAs, camelThrough,
    } = options;
    _.defaults(options, {
    ...
    attributes(model)[levelFieldName] = {
    ...
    const { throughKey, throughForeignKey } = options;
    const throughFields = {};
    ...
    const ancestorsAssociationAccessors = associations[ancestorsAs].accessors;
    const descendentsAssociationAccessors = associations[descendentsAs].accessors;
