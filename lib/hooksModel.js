...
module.exports = (Sequelize, patches) => {
  ...
  const { findOne, findAll, query } = patches;
  ...
  function* beforeCreate(item, options) {
    ...
    const { primaryKey, foreignKey, levelFieldName } = model.hierarchy;
  ...
  function* _beforeUpdate(item, options) {
    ...
    const itemId = values[primaryKey];
    const parentId = values[foreignKey];
  ...
  function* beforeBulkUpdate(options) {
    ...
    const parentId = attributes[foreignKey];
    let level;
    if (parentId === null) {
      level = 1;
    } else {
